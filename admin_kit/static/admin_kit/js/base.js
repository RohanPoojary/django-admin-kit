(function ($) {
  $.fn.kitAttr = function (key, value) {
    ...
    if (value == undefined) return attrObj[key];
    attrObj[key] = value;
  ...
  $.fn.kitFind = function (config) {
    return this.find(".admin-kit").filter(function () {
      for (var key in config) {
        if ($(this).kitAttr(key) != config[key]) return false;
      }
      return true;
    });
  };
    ...
    target_array.forEach(function (target) {
      ...
      $.ajax({
        ...
        success: function (data) {
          ...
          if (data.no_return != undefined || data.no_return == true) return;
          ...
          if (targetElement != undefined) {
            ...
            for (var i = 0; i < elements.length; i++) {
              SetSelectField(
                elements.eq(i),
                data,
                getInitialValues(elements.eq(i))
              );
            }
          }
        },
  ...
  function InitializeAdminKit(element, data, update) {
    if (element.hasClass("admin-kit-select")) {
      if (update != true) InitializeSelect(element, data);
  ...
  function getInitialValues(element) {
    values = {};
    if (
      element.kitAttr("init-value") != undefined &&
      element.kitAttr("init-value") != ""
    ) {
      ...
      if (window.AdminKitConfig.duplicate) {
        if (parent.attr("data-duplicate")) {
          ...
          for (var i = 0; i < children.length; i++) {
            ...
            var total_forms_elem = prev_module.children(
            ...
            var cur_id = curr_module
              .children("input[name$=TOTAL_FORMS]")
              .attr("id")
              .replace("-TOTAL_FORMS", "");
            var prev_id = prev_module
              .children("input[name$=TOTAL_FORMS]")
              .attr("id")
              .replace("-TOTAL_FORMS", "");
  ...
  function duplicateRow($row, $prev, cur_id, prev_id) {
    ...
    for (var i = 0; i < prev_inputs.length; i++) {
      ...
      if (prevName) prevEleInd[prevName] = i;
    }
...
function getSuffixName(ele, suf_id) {
  ...
  if (ele.attr("type") == "hidden") return undefined;
